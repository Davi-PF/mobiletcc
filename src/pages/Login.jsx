    ...
    if ((emailValue && passwordValue) != '') {
      try {
        const response = await axios.get(
          `https://zlospringapplication.online/api/responsibles/findResponsiblesCpfAndName/params?emailRes=${emailValue}&senhaRes=${passwordValue}`,
        );
        let idRes = response.data[0][0];
        let nomeRes = response.data[0][1];

        if (response) {
          updateUserType([{}, true, idRes, nomeRes]);
