...
export default function CodeCheck() {
    ...
    const smsHandlerFunction = async () => {
        try {
            console.log("SMS Data: ", smsData.phoneUser)
            const response = await axios.post('http://localhost:8080/api/smshandler/', smsData);
    ...
    const smsVerifyFunction = async (smsCode) => {
        try {
            const response = await axios.get(`http://localhost:8080/api/smshandler/verifySmsCode?smsCode=${smsCode}&returnDate=${getFunctions.generateTimestamp()}&cpfDep=${smsData.cpfDep}`);
